...
KaynakcaYonetimi:
	g++ -lsqlite3 -lwxcode_`wx-config --basename | cut -d'_' -f 2`_wxsqlite3-`wx-config --release` `wx-config --cxxflags --libs` -o KaynakcaYonetimi src/main.cpp src/main.h src/kaynakcayonetimi.cpp src/kaynakcayonetimi.h src/eklekitap.cpp src/eklekitap.h src/eklemakale.cpp src/eklemakale.h src/buldoi.cpp src/buldoi.h src/md5.cpp src/md5.h src/wxlibs.h
install:
